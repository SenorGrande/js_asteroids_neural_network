...
class NeuralNetwork {
    ...
    train(inputArray, targetArray) {
        ...
        let weights1T = Matrix.transpose(this.weights1); // T for transpose
        let hiddenErrors = Matrix.dot(outputDeltas, weights1T);

        // caluclate the hidden deltas (errors * derivitive of hidden)
...
function sigmoid(x, deriv = false) {
    if (deriv) {
        return x * (1 - x); // wherer x = sigmoid(x)
    }
...
class Matrix {
    constructor(rows, cols, data = []) {
        ...
        if (data == null || data.length == 0) {
            ...
            if (data.length != rows || data[0].length != cols) {
                throw new Error("Incorrect data dimentsions!");
            }
        }
    }
    ...
    // multiply two maatrices (not the dot product)
    ...
    static checkDimensions(m0, m1) {
        if (m0.rows != m1.rows || m0.cols != m1.cols) {
            throw new Error("Matrices are of different dimentsions!");
        }
    }
